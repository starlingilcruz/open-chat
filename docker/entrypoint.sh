...
if ! getent hosts ${REDIS_HOST_VALUE} > /dev/null 2>&1; then
  echo "⚠️  DNS resolution failed for ${REDIS_HOST_VALUE}"
...
if command -v nc >/dev/null 2>&1; then
  if ! nc -z -w 2 ${REDIS_HOST_VALUE} ${REDIS_PORT_VALUE} 2>/dev/null; then
    echo "⚠️  Port ${REDIS_PORT_VALUE} not reachable on ${REDIS_HOST_VALUE}"
  else
...
until redis-cli -h ${REDIS_HOST_VALUE} -p ${REDIS_PORT_VALUE} ping > /dev/null 2>&1; do
  ...
  if [ $((RETRY_COUNT % 10)) -eq 0 ]; then
    ...
    getent hosts ${REDIS_HOST_VALUE} > /dev/null 2>&1 || echo "   ⚠️  DNS still not resolving"
  fi
  sleep 1
done
